...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.qwen2_vl.modeling_qwen2_vl import (F, Callable, Any, Optional, torch, nn, LayerNorm, init, ACT2FN, Cache, GenerationMixin, FlashAttentionKwargs, ModelOutput, BaseModelOutputWithPooling, ROPE_INIT_FUNCTIONS, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, can_return_tuple, is_flash_attention_requested, maybe_autocast, Qwen2VLConfig, Qwen2VLTextConfig, Qwen2VLVisionConfig, logger, __name__, Qwen2VLModel, Qwen2VLModelOutputWithPast, Qwen2VLCausalLMOutputWithPast, Qwen2VLPreTrainedModel, Qwen2VLForConditionalGeneration)

@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def Qwen2VLRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class Qwen2VLRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return Qwen2VLRMSNorm_forward(self, hidden_states=hidden_states)
...
class Qwen2VLRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return Qwen2VLRotaryEmbedding_forward(self, x=x, position_ids=position_ids)
...
@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def VisionRotaryEmbedding_forward(self, seqlen: int) -> torch.Tensor:
...
class VisionRotaryEmbedding(nn.Module):
    ...
    def forward(self, seqlen: int) -> torch.Tensor:
        return VisionRotaryEmbedding_forward(self, seqlen=seqlen)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return PatchEmbed_forward(self, hidden_states=hidden_states)
...
class PatchMerger(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return PatchMerger_forward(self, x=x)
...
class VisionMlp(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        return VisionMlp_forward(self, x=x)
...
class VisionAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return VisionAttention_forward(self, hidden_states=hidden_states, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb, position_embeddings=position_embeddings, **kwargs)
...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return Qwen2VLAttention_forward(self, hidden_states=hidden_states, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, output_attentions=output_attentions, use_cache=use_cache, position_embeddings=position_embeddings, **kwargs)
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Qwen2VLForConditionalGeneration_forward(self, input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, labels=labels, use_cache=use_cache, pixel_values=pixel_values, pixel_values_videos=pixel_values_videos, image_grid_thw=image_grid_thw, video_grid_thw=video_grid_thw, rope_deltas=rope_deltas, mm_token_type_ids=mm_token_type_ids, logits_to_keep=logits_to_keep, **kwargs)
